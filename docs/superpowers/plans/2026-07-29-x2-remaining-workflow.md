...
1. **Controller writes Step 1 of each task** (the failing test / first move), runs it, confirms it fails _for the right reason_, and commits it as `test: … (currently red)`.
...
### 2. Modal component — _step 1 done, finish it_
...
Full `call.ts` and the 9-test suite are in the plan. Manual check: two profiles — both hear audio, hang-up clears _both_ UIs, decline releases the caller.
