...
export const GET: RequestHandler = async ({ locals }) => {
	...
	return json({ mutes: await list_mutes(env, locals.x2_ws, locals.user.id) });
};
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	return json({ mute: await mute(env, locals.x2_ws, locals.user.id, target, kind, until) });
};
...
export const DELETE: RequestHandler = async ({ url, locals }) => {
	...
	if (!target) throw error(400, 'target required');
	await unmute(env, locals.x2_ws, locals.user.id, target);
	return json({ ok: true });
};
