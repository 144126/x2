...
import { put_image, put_file, delete_image, media_url, sign_key, MAX_BYTES, MAX_FILE_BYTES } from '../media';
...
describe('put_file', () => {
	it('accepts allow-listed document types and keeps the original filename, returns a signed URL', async () => {
		const b = bucket();
		const saved = await put_file(b as never, 'user123', img('application/pdf'), 'resume.pdf', 'test-secret');
		expect(saved?.key.startsWith('user123/')).toBe(true);
	...
	it('rejects types outside the allow-list, empty files and oversized files', async () => {
		...
		expect(await put_file(b as never, 'u', img('application/pdf', MAX_FILE_BYTES + 1), 'f', 's')).toBeNull();
		expect(b.put).not.toHaveBeenCalled();
	});
});
