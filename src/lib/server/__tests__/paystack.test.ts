...
describe('paystack_init', () => {
	...
	beforeEach(() => {
		fetchMock = vi.fn().mockResolvedValue(
			new Response(
				JSON.stringify({
					status: true,
					data: { authorization_url: 'https://checkout.paystack.com/abc', access_code: 'ac', reference: 'ref1' }
				}),
				{ status: 200 }
			)
		);
	...
	it('POSTs to /transaction/initialize with amount in kobo and returns the checkout url', async () => {
		...
		expect(r).toEqual({ authorization_url: 'https://checkout.paystack.com/abc', access_code: 'ac', reference: 'ref1' });
		const [url, init] = fetchMock.mock.calls[0];
		expect(url).toBe('https://api.paystack.co/transaction/initialize');
		expect(init.headers.Authorization).toBe('Bearer sk_test_abc');
		const body = JSON.parse(init.body);
		expect(body).toMatchObject({ email: 'a@b.com', amount: 10000, reference: 'ref1', callback_url: 'https://x/callback' });
	});

	it('rejects on a non-ok paystack response', async () => {
		fetchMock.mockResolvedValue(new Response(JSON.stringify({ status: false, message: 'bad' }), { status: 400 }));
		await expect(paystack_init(env, 'a@b.com', 10000, 'ref1', 'https://x/callback')).rejects.toThrow();
	});
});
	...
	it('GETs /transaction/verify/:reference', async () => {
		const fetchMock = vi.fn().mockResolvedValue(
			new Response(
				JSON.stringify({
					status: true,
					data: { status: 'success', reference: 'ref1', amount: 10000, customer: { email: 'a@b.com' }, metadata: {} }
				})
			)
		);
		vi.stubGlobal('fetch', fetchMock);
